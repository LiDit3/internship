# Задание 4: Развёртывание nginx, настройка виртуального хоста и html страницы
...
- **Доступ с хоста:** Если используете `Bridged Adapter`, узнайте IP ВМ через `ip a` и обращайтесь по `http://<VM_IP>:8090`. При NAT — только через проброс портов на `127.0.0.1`. [Проброс портов в VB](/tasks/04-nginx-vhost/assets/04-virtualbox-port-forward.png)
