...
## Чек-лист выполнения
- [ ] Создан отдельный пользователь (например, `intern` или `devuser`)
- [ ] Установлен и активирован SSH-сервер
- [ ] В `/etc/ssh/sshd_config` установлено `PermitRootLogin no`
- [ ] Сервис SSH перезапущен, статус `active (running)`
- [ ] Подключение через `PuTTY` успешно (под новым пользователем)
- [ ] `Notepad++` настроен с плагином `NppFTP` для SFTP
- [ ] `WinSCP` подключается и позволяет просматривать/передавать файлы
- [ ] Ключевые команды, конфиги и скриншоты задокументированы
